...
#define CHECK_CL_PARAM(expr) do { cl_int err = CL_SUCCESS; (expr); if (err != CL_SUCCESS) {  return false; } } while(0)
#define CHECK_CL_RETURN(expr) do { cl_int err = (expr); if (err != CL_SUCCESS) {  return false; } } while(0)
#define CHECK_CL_ON_FAIL(expr, on_fail) do { cl_int err = (expr); if (err != CL_SUCCESS) { on_fail; return false; } } while(0)
...
bool DepthRegistrationOpenCL::init(const cv::Mat &cameraMatrixRegistered, const cv::Size &sizeRegistered, const cv::Mat &cameraMatrixDepth, const cv::Size &sizeDepth,
...
bool DepthRegistrationOpenCL::init(const int deviceId)
{
	...
	if(platforms.empty())
	{
		cout << "no opencl platform available" << endl;
		return false;
	}
	...
	cout << "devices:" << endl;	
	for(size_t i = 0; i < devices.size(); ++i)		
		cout << "  " << i << ": "<< deviceString(devices[i]) << endl;
	
	if(!selectDevice(devices, data->device, deviceId))
	{
		cout << "could not find any suitable device" << endl;
		return false;
	}
	cout << "selected device: " << deviceString(data->device) << endl;
	...
	cl::Program::Sources source(1, std::make_pair(sourceCode.c_str(), sourceCode.length()));
  	...
  	CHECK_CL_ON_FAIL(data->program.build(options.c_str()),
					cout << "failed to build program: " << err << endl;
					cout << "Build Status: " << data->program.getBuildInfo<CL_PROGRAM_BUILD_STATUS>(data->device) << endl;
					cout << "Build Options:\t" << data->program.getBuildInfo<CL_PROGRAM_BUILD_OPTIONS>(data->device) << endl;
					cout << "Build Log:\t " << data->program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(data->device) << endl;);
